...
export const FavoriteButton = (id, onToggle = null) => {
    ...
    const renderIcon = (active) => {
        btn.innerHTML = `
            <svg width="20" height="20" viewBox="0 0 24 24" fill="${active ? 'var(--color-favorite)' : 'none'}" stroke="${active ? 'var(--color-favorite)' : 'currentColor'}" stroke-width="2">
    ...
    btn.addEventListener('click', (e) => {
        ...
        btn.className = `fav-btn ${newStatus ? 'active' : ''}`;
